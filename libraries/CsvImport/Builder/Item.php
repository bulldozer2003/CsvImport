 ...
 * Build an item.
 *
 * @internal Same as Builder_Item, but allows to post data.
 *
 * @see Builder_Item
  *
 * @package CsvImport
 */
class CsvImport_Builder_Item extends Omeka_Record_Builder_AbstractBuilder
{
    const TAGS = 'tags';
    const FILES = 'files';
    ...
    /**
     * Set the post data for the item.
     *
    ...
    /**
     * Add post data to a record.
     */
    private function _addPostData()
    {
        ...
        if (Zend_Registry::get('bootstrap')->config->jobs->dispatcher->longRunning
            ...
            $this->_record->setPostData($this->_postData);
        }
        // Workaround for asynchronous jobs.
        else {
            $this->_setPostDataViaSetArray();
        }
    }
     ...
     *
     * @internal Omeka jobs don't manage ACL, so an issue can occur if a plugin
     * uses it. This is rare, but this is the case in Item::filterPostData(). So
     * if the longrunning process is not the synchronous one, where the used ACL
     * is the current one, this workaround is used.
     *
     * @todo Check if the ACL can be used in asynchronous jobs.
     *
     * @see Item::filterPostData()
     * @see Omeka_Record_Builder_AbstractBuilder::setPostData()
     */
    private function _setPostDataViaSetArray()
    {
        $post = $this->_postData;

        // The Item::filterPostData() is copied here, except lines with ACL.
        // This is not a security issue, because currently, only SuperUser
        // can use CsvImport.
        ...
        $this->_record->setArray(array('_postData' => $post));
    }
